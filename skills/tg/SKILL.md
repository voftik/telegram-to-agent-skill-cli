...
9. **Missing CLI:** if `tg` is not on PATH (skill installed via the
   plugin marketplace), tell the user to run
   `uv tool install telegram-to-agent-skill-cli && tg setup`
   (or `npx telegram-to-agent-skill-cli`) and stop until it is installed.
10. **Maintenance:** when `tg status --yaml` reports
