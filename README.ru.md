<div align="center">

<img src="docs/assets/hero.svg" alt="telegram-to-agent-skill-cli" width="100%"/>

[English](README.md) · **Русский**

[![CI](https://github.com/voftik/telegram-to-agent-skill-cli/actions/workflows/ci.yml/badge.svg)](https://github.com/voftik/telegram-to-agent-skill-cli/actions/workflows/ci.yml)
[![PyPI](https://img.shields.io/pypi/v/telegram-to-agent-skill-cli)](https://pypi.org/project/telegram-to-agent-skill-cli/)
[![npm](https://img.shields.io/npm/v/telegram-to-agent-skill-cli)](https://www.npmjs.com/package/telegram-to-agent-skill-cli)
[![License: Apache-2.0](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](LICENSE)
![Python 3.10+](https://img.shields.io/badge/Python-3.10%2B-3776AB?logo=python&logoColor=white)

*Спроси агента «что обсуждали в рабочем чате на этой неделе?», и он ответит по делу.*

</div>

---

Половина рабочего контекста живёт в Telegram: команда принимает решения в чатах, коллеги кидают ТЗ файлами и ссылки на доки, которые никто не перенёс в вики. Этот инструмент отдаёт весь этот контекст Claude Code, Codex и любому агенту, который умеет запускать команды.

CLI `tg` входит в Telegram твоим аккаунтом (MTProto через [Telethon](https://github.com/LonamiWebs/Telethon)), складывает чаты в локальную базу SQLite с полнотекстовым поиском и ставит навык, который агент включает сам, как только ты упоминаешь свои чаты. Агент читает локальную базу: мгновенно, офлайн, без лимитов Telegram. К самому Telegram инструмент ходит только за синхронизацией, файлами и отправкой ответа, причём отправит только после твоего явного «да».

<div align="center">
<img src="docs/assets/demo.svg" alt="Демо агентской сессии" width="80%"/>
</div>

## Установка

Выбери любой из трёх путей. Каждый заканчивается интерактивным визардом: вход, навык агентам, настольные приложения, первая синхронизация. API-ключи не нужны: нажми Enter, и подключатся встроенные ключи Telegram Desktop (свои с my.telegram.org по-прежнему рекомендуются при интенсивной синхронизации).

```bash
# через uv (рекомендую)
uv tool install telegram-to-agent-skill-cli && tg setup
```

```bash
# разовый запуск без установки
uvx --from telegram-to-agent-skill-cli tg setup
```

```bash
# через npm, если тебе привычнее Node
npx telegram-to-agent-skill-cli
```

Разработчики клонируют репозиторий и запускают `./install.sh`: тот ставит editable-версию и открывает тот же визард. Подробности: [docs/INSTALL.md](docs/INSTALL.md).

Пользователи Claude Code могут подключить репозиторий как маркетплейс плагинов, тогда навык ставится без терминала:

```
/plugin marketplace add voftik/telegram-to-agent-skill-cli
/plugin install telegram-context@telegram-to-agent-skill-cli
```

Сам CLI приходит с PyPI: если команда `tg` не найдена, навык подскажет команду установки.

## Обновление

```bash
tg update          # проверит PyPI, обновит CLI и навык
tg update --check  # только проверка; агенты смотрят update.update_available в tg status --yaml
```

Команды с данными в сеть за версиями не ходят: подсказка о новой версии читает локальный кэш и печатается в stderr только в живом терминале. Переменная `TG_UPDATE_CHECK=0` отключает её совсем.

## Настольные чат-приложения

Claude Desktop, Perplexity и ChatGPT в режиме чата не умеют запускать CLI, зато умеют локальные MCP-серверы. Одна команда подключает их к тому же локальному индексу через `tg mcp`, read-only мост, который не трогает Telegram-сессию:

```bash
tg connect
```

Она находит приложения, прописывает их конфиги, сама проверяет мост и предлагает `tg autosync`: фоновое обновление индекса по расписанию. Разбор по каждому приложению: [docs/DESKTOP-APPS.md](docs/DESKTOP-APPS.md).

## Почему сначала навык (и где место MCP-мосту)

- **Контекст остаётся агенту.** Схемы MCP-инструментов съедают токены в каждой сессии. Навык подгружается по требованию, а CLI не стоит ничего, пока его не позвали. Кодинг-агенты (Claude Code, Codex) получают полный навык плюс CLI.
- **Одна интеграция на всех.** Те же команды `tg` работают в Claude Code, Codex и везде, где есть терминал.
- **Никакой борьбы за сессию.** Синхронизацией владеет один процесс CLI на хосте, а читают SQLite-индекс сколько угодно сессий. Встроенный мост `tg mcp` живёт по тому же правилу: read-only адаптер для хостов без терминала, шесть маленьких инструментов, без Telegram-сессии, без отправки.

## Как это устроено

```mermaid
flowchart LR
    TG[("Telegram<br/>(твой аккаунт, MTProto)")]
    subgraph LOCAL["Твоя машина"]
        SYNC["tg sync / refresh<br/><i>по требованию</i>"]
        DB[("SQLite + FTS5<br/>сообщения · вложения · ссылки · треды")]
        FILES["files/ + извлечённый текст<br/>pdf · docx · xlsx · pptx"]
        QUERY["tg brief · search · thread<br/>links · files · style"]
    end
    subgraph AGENTS["Агенты"]
        CC["Claude Code<br/><i>навык включается сам</i>"]
        CX["Codex<br/><i>через AGENTS.md</i>"]
    end
    TG -->|история| SYNC --> DB
    TG -->|ленивое скачивание| FILES
    DB --> QUERY
    QUERY --> CC & CX
    FILES --> CC & CX
    CC & CX -.->|"отправка только с --confirm<br/>после явного «да»"| TG
```

## Что агент умеет с этим делать

| Просьба обычным языком | Что агент делает под капотом |
| --- | --- |
| «Что обсуждали в чате проекта?» | синхронизирует чат, оценивает объём через `brief`, читает `recent` и пишет саммари с датами и авторами |
| «Найди, где кидали док с ценами» | `tg links --kind gdoc` отдаёт export-ссылку: чистый текст вместо JS-обёртки |
| «Прочитай спеку, которую скинули файлом» | `tg files --download` кладёт рядом с файлом извлечённый текст |
| «Восстанови тот спор про дедлайн» | `tg thread` собирает цепочку ответов, даже если корень треда это опрос |
| «Напиши ответ моим голосом» | `tg style` даёт агенту корпус твоих сообщений; черновик остаётся черновиком, пока ты не скажешь «да» |
| «Дайджест рабочих чатов со вчера» | агент обходит чаты, собирает главное и помечает, что ждёт твоей реакции |

## Безопасность

- Session-файл Telethon равен полному доступу к аккаунту. Он лежит в закрытом каталоге (права 0700/0600), не попадает в git и облачные папки; каждая машина входит в аккаунт отдельно.
- Любая запись в Telegram (`send`, `edit`, `delete`) без флага `--confirm` остаётся сухим прогоном. Подтверждённые действия CLI сначала записывает в журнал, потом выполняет.
- Чужие вложения проходят проверки: лимит размера до скачивания, защита от zip-бомб, потоковое хэширование, закрытые права на файлы.
- Заведи собственные `api_id` и `api_hash`. Инструмент синхронизирует вежливо: задержки, джиттер, обработка FloodWait.

## Сравнение с альтернативами

Честное сравнение с другими способами отдать агенту твой Telegram (состояние экосистемы на август 2026):

| | **tg (этот проект)** | Telegram MCP-серверы¹ | Исходный tg-cli | Экспорт из Telegram Desktop |
| --- | --- | --- | --- | --- |
| Работает в любом CLI-агенте | да, одна установка | отдельный MCP-конфиг на каждого агента | да | ручной перенос |
| Цена в контексте сессии | ноль, пока не позвали | схемы инструментов съедают токены в каждой сессии | ноль | ноль |
| Поиск по всей истории | FTS5, миллисекунды, офлайн | живые API-вызовы с лимитами | скан `LIKE` | нет (статичные файлы) |
| Вложения как читаемый текст | pdf/docx/xlsx/pptx извлекаются | в лучшем случае скачивание | не хранит | сырые файлы |
| Ссылки на Google Docs | готовые export-ссылки | нет | нет | нет |
| Восстановление тредов | да, включая ссылки t.me | частично | нет | нет |
| Параллельные сессии агентов | читателей сколько угодно | конфликты за session-файл² | один пользователь | не применимо |
| Безопасность отправки | сухой прогон, `--confirm`, журнал | по-разному; часть шлёт сразу | шлёт сразу | не применимо |
| Целостность синка | курсоры дыр, `tg backfill` | не применимо (живое чтение) | как получится | разовый срез |
| Свежесть данных | инкрементальный синк за секунды | всегда живые | инкрементальный | заморожена на момент экспорта |
| Установка и обновление | однострочник `npx`/`uv`, `tg update` | ручная настройка сервера | pip | встроено в приложение |

¹ chigwell/telegram-mcp, chaindead/telegram-mcp, overpod/mcp-telegram и похожие. Они уместны, когда агент живёт в веб-версии claude.ai, которая вообще не может запускать локальные процессы; а режим «только черновики» у chaindead придуман по-настоящему безопасно. Настольные приложения (Claude Desktop, Perplexity, ChatGPT desktop) закрывает встроенный read-only мост `tg mcp`.
² Telethon и GramJS пускают один процесс на session-файл; MCP-серверы плодятся по сессии на агента и сталкиваются (обходные схемы с общим демоном требуют отдельной настройки).

## Что форк добавил к исходному tg-cli

| Область | [upstream](https://github.com/jackwener/tg-cli) | этот форк |
| --- | --- | --- |
| Вложения | не хранит | индексирует при синке, качает лениво, извлекает текст (pdf/docx/xlsx/pptx/csv) |
| Ссылки | не извлекает | `tg links` с готовой ссылкой для агента, структурный разбор URL |
| Треды | нет | `tg thread` собирает цепочки, понимает ссылки t.me |
| Поиск | скан через `LIKE` | FTS5 плюс юникодный фолбэк и полный обход в regex-поиске |
| Целостность синка | как получится | курсоры дыр, `tg backfill`, честный отчёт по каждому чату |
| Идентификаторы | голые ID сталкиваются | marked ID везде, ленивая миграция старых баз |
| Первый синк | вручную | `tg bootstrap` переживает перезагрузки и убирает себя сам |
| Отправка | шлёт сразу | сухой прогон по умолчанию, `--confirm`, журнал операций |
| Интеграция с агентами | файл с документацией | навык в пакете, визард установки, самообновление, автовключение, MCP-мост для настольных чат-приложений |

## Благодарности

Форк [jackwener/tg-cli](https://github.com/jackwener/tg-cli) (Apache-2.0): чистое local-first ядро придумали они. Работает на [Telethon](https://github.com/LonamiWebs/Telethon). Лицензия: [Apache-2.0](LICENSE).
